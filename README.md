...
1. **Unitary Product Equivalence:** Applying the Hadamard intertwining identity $X H = H Z$:
   ...
   Both evaluate to the exact same transformation matrix:
...
4. **Empirical Measurement Statistics:** Across 1024 shots on the `AerSimulator`, both circuits display balanced binomial readout distributions matching theoretical expectations ($P=0.50$):
...
- **Step 1 ($X$ gate):** $|\psi_1\rangle = X|0\rangle = |1\rangle$
- **Step 2 ($H$ gate):** $|\psi_2\rangle = H|1\rangle = |-\rangle = \frac{1}{\sqrt{2}}(|0\rangle - |1\rangle)$
- **Step 3 ($Z$ gate):** $|\psi_3\rangle = Z|-\rangle = |+\rangle = \frac{1}{\sqrt{2}}(|0\rangle + |1\rangle)$
- **Step 4 ($X$ gate):** $|\psi_4\rangle = X|+\rangle = |+\rangle = \frac{1}{\sqrt{2}}(|0\rangle + |1\rangle)$
...
| :--- | :--- | :--- |
| **Statevector** | $\frac{1}{2}(\vert 00 \rangle + \vert 01 \rangle + \vert 10 \rangle + \vert 11 \rangle)$ | $\frac{1}{\sqrt{2}}(\vert 00 \rangle + \vert 11 \rangle)$ |
| **Separability** | Separable: $\vert + \rangle_1 \otimes \vert + \rangle_0$ | Non-Separable (Entangled) |
| **Schmidt Rank ($r$)** | $r = 1$ | $r = 2$ |
| **Reduced Density Matrix $\rho_0$** | Pure: $\vert + \rangle\langle + \vert$ | Maximally Mixed: $\frac{1}{2}I_2$ |
...
- **Superposition:** $(I \otimes H)|00\rangle = \frac{1}{\sqrt{2}}(|00\rangle + |01\rangle)$
...
| :---: | :---: | :---: | :---: |
| **$\Phi^+$** | $\frac{1}{\sqrt{2}}(\vert 00 \rangle + \vert 11 \rangle)$ | $\{00, 11\}$ | $00: 510, \; 11: 514$ |
| **$\Phi^-$** | $\frac{1}{\sqrt{2}}(\vert 00 \rangle - \vert 11 \rangle)$ | $\{00, 11\}$ | $00: 510, \; 11: 514$ |
...
| :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **1** | $H_0 \to CX_{(0 \to 1)}$ | $\frac{1}{\sqrt{2}}(\vert 00 \rangle + \vert 11 \rangle)$ | **Entangled** | $r = 2$ | $1.0000\text{ ebit}$ | $00: 503, \; 11: 521$ |
...
5. **Computational-Basis Dimensional Scaling:**  
   ...
   **Mathematical Foundation:** The state space of an $n$-qubit register is the tensor product of $n$ two-dimensional single-qubit spaces:
   
   $$
   \mathcal{H}^{\otimes n} = \bigotimes_{k=1}^n \mathbb{C}^2 \cong \mathbb{C}^{2^n}
   $$
   
   The dimension of the composite product space equals the product of subsystem dimensions: $\dim(\mathcal{H}^{\otimes n}) = \prod_{k=1}^n 2 = 2^n$.
